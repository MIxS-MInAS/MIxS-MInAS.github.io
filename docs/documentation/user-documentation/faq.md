...
Sample collection date may refer to different sampling events. For this reason, three fields are provided for recording date-related information associated with the sequencing of a sample for ancient DNA analysis:

- The `date of retrieval from depositional context (context_retrieval_date)` refers to the date of excavation or retrieval from burial or depositional context, if known. In cases where excavations are carried out over several weeks, months, or years, report its midpoint at a month or year level. If the exact date is unknown, the date can be right truncated to include the year and month, or only the year.
